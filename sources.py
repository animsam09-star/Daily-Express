...
# 섹터 상위 N 규칙만으로는 구조적으로 못 보는 종목들. XLK 는 시총 상위가
# 조 단위 빅테크로 채워져 퀄컴·TI·어도비·세일즈포스·IBM 같은 대표주가
# 통째로 잘린다(실측: 오늘 페이지에 CRM·QCOM·TXN·ADBE·IBM 이 없었다).
# 여기 올린 종목은 시총·거래량 하한과 주도주 조건을 무시하고 항상 실린다.
# 값은 소속 섹터 — 섹터 카드에 ☆ 로 붙는 자리를 정한다.
WATCHLIST = {
    "PLTR": "XLK",      # 팔란티어 — XLK 시총 12위라 상위 5 규칙에 늘 잘린다
    "BE": "XLI",        # 블룸에너지 — 지수 미편입. GICS 기준 산업재(전기장비)

    # 반도체·장비
    "QCOM": "XLK", "TXN": "XLK", "ADI": "XLK", "MRVL": "XLK",
    "LRCX": "XLK",      # 램리서치
    "KLAC": "XLK",      # KLA
    "ARM": "XLK",       # 암 홀딩스
    "NXPI": "XLK",      # NXP
    # 메모리·스토리지
    "SNDK": "XLK",      # 샌디스크 — 2025년 WDC 에서 분할, 지수 편입 전
    "STX": "XLK",       # 시게이트
    "WDC": "XLK",       # 웨스턴디지털
    # 광통신·AI 인프라
    "LITE": "XLK",      # 루멘텀
