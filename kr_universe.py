...
# {정리 대상 테마: [우선권을 갖는 테마들]} — 겹치면 뒤쪽 테마에만 남긴다
EXCLUSIVE = {"화학·소재": ["2차전지"]}
...
def build_pools() -> dict[str, list[str]]:
    ...
    # 테마가 겹치는 것 자체는 자연스럽다(현대차는 자동차이면서 로봇 밸류체인이다).
    # 다만 전지 밸류체인은 화학·소재보다 2차전지 쪽 정체성이 뚜렷해 한쪽만 남긴다.
    for host, owners in EXCLUSIVE.items():
        taken = {c for o in owners for c in pools.get(o, [])}
        if host in pools:
            pools[host] = [c for c in pools[host] if c not in taken]
